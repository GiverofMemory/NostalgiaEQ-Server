...
my $eqemu_repository_request_url   = "https://raw.githubusercontent.com/vire70/Server/master/";
my $opcodes_path                   = "";
my $patches_path                   = "";
...
sub check_xml_to_json_conversion
{
    if (-e "eqemu_config.xml" && !-e "eqemu_config.json") {
        ...
        if ($OS eq "Windows") {
            get_remote_file("https://raw.githubusercontent.com/vire70/server/master/utils/xmltojson/xmltojson-windows-x86.exe",
                "xmltojson.exe");
            print "Converting eqemu_config.xml to eqemu_config.json\n";
            print `xmltojson eqemu_config.xml`;
        }
        if ($OS eq "Linux") {
            get_remote_file("https://raw.githubusercontent.com/vire70/server/master/utils/xmltojson/xmltojson-linux-x86",
                "xmltojson");
            print "Converting eqemu_config.xml to eqemu_config.json\n";
...
sub build_linux_source
{
    ...
    if (!-d "$source_dir/.git") {
        print `git clone --recurse-submodules https://github.com/vire70/server.git $source_dir`;
    }
    else {
...
sub get_remote_file
{
    ...
    #::: wget -O db_update/db_update_manifest.txt https://raw.githubusercontent.com/vire70/server/master/utils/sql/db_update_manifest.txt
    if ($OS eq "Linux") {
...
{
    if (!-e "updates_staged/FVProject-Quests-master/") {
        print "[Update] Fetching Latest Quests --- \n";
        get_remote_file("https://codeload.github.com/Gates-Of-Time/FVProject-Quests/zip/master",
            "updates_staged/FVProject-Quests-master.zip",
            1);
        print "[Install] Fetched latest quests...\n";
        mkdir('updates_staged');
        unzip('updates_staged/FVProject-Quests-master.zip', 'updates_staged/');
    }
    ...
    my @files;
    my $start_dir = "updates_staged/FVProject-Quests-master/";
    find(
        sub {push @files, $File::Find::name unless -d;},
        $start_dir
    );
    for my $file (@files) {
        if ($file =~ /\.pl|\.lua|\.ext/i) {
            $staged_file      = $file;
            $destination_file = $file;
            $destination_file =~ s/updates_staged\/FVProject-Quests-master\//quests\//g;
...
sub lua_modules_fetch
{
    ...
    my @files;
    my $start_dir = "updates_staged/FVProject-Quests-master/lua_modules/";
    find(
        sub {push @files, $File::Find::name unless -d;},
        $start_dir
    );
    for my $file (@files) {
        if ($file =~ /\.pl|\.lua|\.ext/i) {
            $staged_file      = $file;
            $destination_file = $file;
            $destination_file =~ s/updates_staged\/FVProject-Quests-master\/lua_modules\//lua_modules\//g;
...
sub plugins_fetch
{
    ...
    my @files;
    my $start_dir = "updates_staged/FVProject-Quests-master/plugins/";
    find(
        sub {push @files, $File::Find::name unless -d;},
        $start_dir
    );
    for my $file (@files) {
        if ($file =~ /\.pl|\.lua|\.ext/i) {
            $staged_file      = $file;
            $destination_file = $file;
            $destination_file =~ s/updates_staged\/FVProject-Quests-master\///g;
